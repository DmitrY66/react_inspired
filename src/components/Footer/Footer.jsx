      ...
      <Container>
        <div className={st.container}>
          <div className={st.category}>
            <h2 className={cn(st.title, st.categoryTitle)}>Каталог</h2>
            <ul className={st.categoryList}>
              {genderList.map(gender => (
                <li key={gender} className={st.categoryItem}>
                  <h3 className={st.categorySubtitle}>
                    <NavLink className={st.link} to={gender}>{categories[gender].title}</NavLink>
                  </h3>
                  <ul className={st.categorySublist}>
                    {categories[gender].list.map(category => (
                      <li key={category.slug}>
                        <NavLink
                          className={st.link}
                          to={`${gender}/${category.slug}`}>{category.title}</NavLink>
                      </li>
                    ))}
                  </ul>
                </li>
          ...
          <div className={st.social}>
            <h2 className={cn(st.title, st.socialTitle)}>Связаться с нами</h2>

            <p className={st.socialTitle}>Контакты и адреса магазинов</p>

            <ul className={st.socialList}>
              <li><a href='#' className={cn(st.link, st.socialLinkVK)}>
              ...
              <li><a href='#' className={cn(st.link, st.socialLinkFB)}>
                <svg width="24" height="24" viewBox="0 0 24 24" fill="currentColor" xmlns="http://www.w3.org/2000/svg">
          ...
          <div className={st.contacts}>
            <a href='mailto:Inspired@gmail.com' className={st.link}>Inspired@gmail.com</a>
            <a href='tel:89304902620' className={st.link}>8 930 490 26 20</a>
          </div>
          ...
          <div className={st.development}>
            <ul className={st.developmentList}>
              <li className={st.developmentItem}>Designer:
                <a className={st.link} href='https://t.me/Mrshmallowww'>Anastasia Ilina</a></li>
              <li className={st.developmentItem}>Developer:
                <a className={st.link} href='https://t.me/Mrshmallowww'>Dmitrii</a></li>
            </ul>
          </div>
        </div>
      </Container>
